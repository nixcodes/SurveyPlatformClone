...
const PostTraining = () => {
  ...
  useEffect(() => {
    async function fetchImages() {
      try {
        ...
        const bucketUrl = 'https://cytra.s3.us-east-2.amazonaws.com/IOS/don';
  ...
  const handleSubmit = async (event) => {
    ...
    if (unanswered.length > 0) {
      ...
      const sheetName = 'PostTrainingResponsesIos';
      const data = [];
      ...
      const fileName = `PostTrainingResponsesIos_${participantId}.xlsx`;
      ...
      try {
        ...
        navigate(`/instructions/post-survey/${participantID}`);
      } catch (error) {
